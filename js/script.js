...
function initializeFlashDeals() {
    ...
    console.log(`Found ${flashDealsProducts.length} flash deal candidates`);
    
    if (flashDealsProducts.length === 0) {
        console.warn('No flash deals available');
        hideFlashDealsSection();
        return;
    }
...
// Get products with high discount and limited offers
function getFlashDealCandidates() {
    return allProducts.filter(product => {
        // Check for discount
        const discountNum = product.discount ? parseFloat(product.discount) : 0;
        const hasHighDiscount = discountNum >= 25;
        
        // Check for limited offer
        const isLimited = product.limited === true || 
                         product.limited === 'true' || 
                         (product.stock && parseInt(product.stock) < 10);
        
        return hasHighDiscount || isLimited;
    });
}

// Render flash deals products
function renderFlashDeals() {
    ...
    if (currentSlice.length < FLASH_DEALS_COUNT && flashDealsProducts.length > 0) {
        const remaining = FLASH_DEALS_COUNT - currentSlice.length;
        currentSlice = currentSlice.concat(flashDealsProducts.slice(0, remaining));
    }
    ...
    console.log(`Rendered ${currentSlice.length} flash deal products`);
}
...
// Rotate to next set of products
...
// Hide flash deals section if no products
function hideFlashDealsSection() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize flash deals after a short delay
    setTimeout(initializeFlashDeals, 2000);
});
