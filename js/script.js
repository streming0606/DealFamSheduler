...
function getFlashDealCandidates() {
    if (!Array.isArray(allProducts) || allProducts.length === 0) {
        console.log('⚠️ No products available for flash deals');
        return [];
    }
    
    // Sort by posted_date descending (most recent first)
    const sorted = [...allProducts].sort((a, b) => {
        const dateA = new Date(a.posted_date || a.posteddate || 0);
        const dateB = new Date(b.posted_date || b.posteddate || 0);
        return dateB - dateA;
    });
    
    // Get the top 25 or all available if fewer
    const candidates = sorted.slice(0, Math.min(FLASH_DEALS_COUNT, sorted.length));
    console.log(`🔥 Flash deals: Selected ${candidates.length} most recent products`);
    return candidates;
}
...
function renderFlashDeals() {
    ...
    console.log(`🔥 Rendering ${flashDealsProducts.length} flash deals`);
    ...
    flashDealsProducts.forEach((product, index) => {
        try {
            ...
            console.error('❌ Error creating flash deal card:', error);
        }
    });

    // Initialize enhanced features after rendering
    setTimeout(() => {
    ...
    console.log('✅ Flash deals rendered successfully');
}
...
function startFlashDealsCountdown() {
    ...
    if (!timerElement) {
        console.warn('⚠️ Flash deals timer element not found');
        return;
    }
    ...
    flashDealsTimerInterval = setInterval(() => {
        ...
        if (remaining <= 0) {
            clearInterval(flashDealsTimerInterval);
            timerElement.textContent = '🔄 Rotating deals...';
            rotateFlashDeals();
            return;
        }
    ...
    console.log('⏰ Flash deals countdown started');
}

// Fade utilities for smooth transitions
...
function rotateFlashDeals() {
    ...
    fadeOutElement(section, 500, () => {
        ...
        console.log('✅ Flash deals rotation completed');
    });
}

// MAIN INITIALIZATION - Wait until allProducts is available
function initializeFlashDealsWithWait() {
    console.log('🔥 Initializing flash deals...');
    
    // Check if products are available
    if (!allProducts || allProducts.length === 0) {
        console.log('⏳ Flash deals waiting for products...');
        setTimeout(initializeFlashDealsWithWait, 300);
        return;
    }

    console.log(`🔥 Flash deals found ${allProducts.length} products`);
    ...
    console.log('✅ Flash deals initialized successfully');
}

// Integration with main app initialization
function initializeFlashDeals() {
    console.log('🔥 Flash deals module loaded');
...
// Cleanup function (call this if needed)
function cleanupFlashDeals() {
    if (flashDealsTimerInterval) {
        clearInterval(flashDealsTimerInterval);
...
window.addEventListener('beforeunload', cleanupFlashDeals);
